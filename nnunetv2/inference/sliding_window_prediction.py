...
def predict_sliding_window_return_logits3D2D(network: nn.Module,
                                         input_image: Union[np.ndarray, torch.Tensor],
                                         num_segmentation_heads: int,
                                         tile_size: Union[Tuple[int, ...], List[int]],
                                         mirror_axes: Tuple[int, ...] = None,
    ...
    with torch.no_grad():
        ...
        with torch.autocast(device.type, enabled=True) if device.type == 'cuda' else dummy_context():
            ...
            if use_gaussian:
                gaussian = compute_gaussian(tuple(tile_size[1:]), sigma_scale=1. / 8, value_scaling_factor=1000,
                                            device=device) if precomputed_gaussian is None else precomputed_gaussian
            ...
            # preallocate results and num_predictions. Move everything to the correct device
            try:
                predicted_logits = torch.zeros((num_segmentation_heads, 1, *data.shape[2:]), dtype=torch.half,
                                               device=results_device)
                n_predictions = torch.zeros((1, *data.shape[2:]), dtype=torch.half,
                                            device=results_device)
                gaussian = gaussian.to(results_device)
            except RuntimeError:
                # sometimes the stuff is too large for GPUs. In that case fall back to CPU
            ...
            predicted_logits /= n_predictions
    empty_cache(device)
    return predicted_logits[tuple([slice(None), *slicer_revert_padding[1:]])].mean(1) # Mean over x dim to return 2D logits
