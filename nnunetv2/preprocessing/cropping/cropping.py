...
def create_nonzero_mask_3D2D(data):
    """
    ...
    nonzero_mask = np.zeros((1, *data.shape[2:]), dtype=bool)
    for c in range(data.shape[0]):
        this_mask = np.max(data[c] != 0, 0, keepdims=True)
        nonzero_mask = nonzero_mask | this_mask
    nonzero_mask = binary_fill_holes(nonzero_mask)
    return nonzero_mask


def crop_to_nonzero_3D2D(data, seg=None, nonzero_label=-1):
    """
    ...
    :return:
    """
    nonzero_mask = create_nonzero_mask_3D2D(data)
    bbox = get_bbox_from_mask(nonzero_mask)

    slicer = bounding_box_to_slice(bbox)
    data = data[tuple([slice(None), slice(None), *slicer[1:]])]

    if seg is not None:
        seg = seg[tuple([slice(None), slice(None), *slicer[1:]])]
