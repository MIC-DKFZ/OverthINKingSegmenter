...
from nnunetv2.utilities.utils import (
    get_folder_identifiers_from_splitted_dataset_folder,
...
from nnunetv2.preprocessing.normalization.default_normalization_schemes import (
    CT16NormalizationSlice,
    CT16NormalizationSliceStandardize,
)


class Preprocessor3D2DSliceselect(object):
    ...
    def run_case_npy(
        ...
        data, seg, bbox = crop_to_nonzero_3D2D(data, seg)
        ...
        # seg = configuration_manager.resampling_fn_seg(seg, new_shape_seg, original_spacing, target_spacing)
        if self.verbose:
            print(
        ...
        if has_seg:
            ...
            properties["data_locations"] = self._sample_data_locations(seg)
    ...
    def _sample_foreground_locations(
        ...
        for c in classes_or_regions:
            ...
            class_locs[k] = all_locs
        return class_locs

    @staticmethod
    def _sample_data_locations(seg: np.ndarray):
    ...
    def _normalize(
        ...
        for c in range(len(data)):
            normalizer = CT16NormalizationSlice(
                use_mask_for_norm=configuration_manager.use_mask_for_norm[c],
                intensityproperties={},
            )
            data[c] = normalizer.run(data[c][0], seg[0])
        data = np.stack(data)
        assert not np.any(np.isnan(data)), "nan in data after normalization"
        assert not np.any(np.isinf(data)), "inf in data after normalization"
        return data
    ...
    def run(
        ...
        identifiers = get_folder_identifiers_from_splitted_dataset_folder(
        ...
        image_fnames = create_folder_lists_from_splitted_dataset_folder(
            join(nnUNet_raw, dataset_name, "imagesTr"), identifiers
        )
...
class Preprocessor3D2DSliceselect40(Preprocessor3D2DSliceselect):
    def _normalize(
        self,
        data: np.ndarray,
        seg: np.ndarray,
...
class Preprocessor3D2DSliceselectFirst(Preprocessor3D2DSliceselect):
    def _normalize(
        self,
        data: np.ndarray,
        seg: np.ndarray,
...
class Preprocessor3D2DSliceselectStandardize(Preprocessor3D2DSliceselect):
    def _normalize(
        self,
        data: np.ndarray,
        seg: np.ndarray,
        ...
        data = np.split(data, data.shape[0])
        for c in range(len(data)):
            normalizer = CT16NormalizationSliceStandardize(
                use_mask_for_norm=configuration_manager.use_mask_for_norm[c],
                intensityproperties={},
            )
            data[c] = normalizer.run(data[c][0], seg[0])
...
def example_test_case_preprocessing():
    ...
    plans_file = (
        "nnUNet_preprocessed/Dataset763_overthinking_segmenter_full/nnUNetPlans.json"
    )
    dataset_json_file = (
        "nnUNet_preprocessed/Dataset763_overthinking_segmenter_full/dataset.json"
    )
    input_images = [
        "nnUNet_raw/Dataset763_overthinking_segmenter_full/imagesTr/scroll_2_0000",
    ]
    seg_file = "nnUNet_raw/Dataset763_overthinking_segmenter_full/labelsTr/scroll_2.tif"

    configuration = "3d_fullres"
    pp = Preprocessor3D2DSliceselect()
