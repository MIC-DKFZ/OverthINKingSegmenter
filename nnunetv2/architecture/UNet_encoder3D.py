...
class UNetEncoder3DSigmoid(nn.Module):
    def __init__(self,
                 input_channels: int,
                 n_stages: int,
                 features_per_stage: Union[int, List[int], Tuple[int, ...]],
        ...
        for s in range(n_stages):
            ...
            if pool == 'max' or pool == 'avg':
                ...
                raise RuntimeError()
            stage_modules.append(StackedConvBlocksSigmoid(
                n_conv_per_stage[s], conv_op, input_channels, features_per_stage[s], kernel_sizes[s], conv_stride,
                conv_bias, norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs, nonlin_first
            ))
            stages.append(nn.Sequential(*stage_modules))
...
class StackedConvBlocksSigmoid(nn.Module):
    def __init__(self,
                 num_convs: int,
        ...
        self.convs = nn.Sequential(
            ...
            *[
                ConvDropoutNormReLU(
                    ...
                    norm_op_kwargs, dropout_op, dropout_op_kwargs, nn.Sigmoid, None, nonlin_first
                )
        ...
        self.second_conv = ConvDropoutNormReLU(
                    conv_op, output_channels[0], output_channels[1], kernel_size, 1, conv_bias, norm_op,
                    norm_op_kwargs, dropout_op, dropout_op_kwargs,nonlin, nonlin_kwargs, nonlin_first
                )
        self.weighting_conv = ConvDropoutNormReLU(
                    conv_op, output_channels[0], output_channels[1], kernel_size, 1, conv_bias, norm_op,
                    norm_op_kwargs, dropout_op, dropout_op_kwargs, nn.Sigmoid, None, nonlin_first
                )
