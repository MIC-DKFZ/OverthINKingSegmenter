...
def get_pool_and_conv_props(spacing, patch_size, min_feature_map_size, max_numpool):
    ...
    while True:
        ...
        for i, d in enumerate(valid_axes_for_pool):
            if kernel_size[d] == 3:
                continue
            else:
                if spacings_of_axes[i] / min(current_spacing) < 2:
                    kernel_size[d] = 3

        # Set kernel sizes to 1 for axes not in valid_axes_for_pool
        for d in range(dim):
            if d not in valid_axes_for_pool:
                kernel_size[d] = 1
