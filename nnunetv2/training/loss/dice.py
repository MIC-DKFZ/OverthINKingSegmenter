...
class MemoryEfficientSoftSorensenDiceLoss(nn.Module):
    def __init__(self, apply_nonlin: Callable = None, batch_dice: bool = False, do_bg: bool = True, smooth: float = 1.,
                 ddp: bool = True, beta: int = 1):
        super(MemoryEfficientSoftSorensenDiceLoss, self).__init__()
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        dc = ((1+self.beta**2) * intersect + self.smooth) / (torch.clip(self.beta**2*sum_gt + sum_pred + self.smooth, 1e-8))
