...
class Preprocessor3D2D(object):
    ...
    def run_case_npy(
        ...
        seg = configuration_manager.resampling_fn_seg(
            seg, new_shape_seg, original_spacing, target_spacing
        )
        if self.verbose:
            print(
...
def example_test_case_preprocessing():
    ...
    plans_file = (
        "nnUNet_preprocessed/Dataset760_overthinking_segmenter_full/nnUNetPlans.json"
    )
    dataset_json_file = (
        "nnUNet_preprocessed/Dataset760_overthinking_segmenter_full/dataset.json"
    )
    input_images = [
        "nnUNet_raw/Dataset760_overthinking_segmenter_full/imagesTr/scroll_2_0000",
    ]
    seg_file = "nnUNet_raw/Dataset760_overthinking_segmenter_full/labelsTr/scroll_2.tif"
