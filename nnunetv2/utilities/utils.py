...
def get_folder_identifiers_from_splitted_dataset_folder(folder: str):
    folders = subfolders(folder, join=False)
    # all files must be .nii.gz and have 4 digit channel index
    crop = 5
    folders = [i[:-crop] for i in folders]
    # only unique image ids
...
def create_folder_lists_from_splitted_dataset_folder(folder: str, identifiers: List[str] = None) -> List[List[str]]:
    """
    does not rely on dataset.json
    """
    if identifiers is None:
        identifiers = get_folder_identifiers_from_splitted_dataset_folder(folder)
    assert isinstance(identifiers, (list,tuple,np.ndarray)), "identifiers must be list or tuple"
    files = subfolders(folder, join=False, sort=True)
    list_of_lists = []
    for f in identifiers:
        p = re.compile(re.escape(f) + r"_\d\d\d\d")
        list_of_lists.append([join(folder, i) for i in files if p.fullmatch(i)])
    return list_of_lists
